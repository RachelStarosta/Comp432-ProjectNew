...
while not done:
    ...
    for i in range(game.height):
        for j in range(game.width):
            ...
            if game.field[i][j] > 0:
                pygame.draw.rect(screen, T.colors[game.field[i][j]],
        ...
        for i in range(4):
            for j in range(4):
                ...
                if p in game.figure.image():
                    pygame.draw.rect(screen, T.colors[game.figure.color],
...
class Learning:
    ...
    backG = pygame.image.load("SampleBackground.png")
    backG = pygame.transform.scale(backG, surfaceSize)
